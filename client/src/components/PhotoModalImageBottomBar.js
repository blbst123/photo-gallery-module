...
const PhotoModalImageBottomBar = function (props) {
  const { photoNum, photoTotalNum } = props;

  return (
    <div className={css['photo-modal_image-bottom-bar']}>
      <div className={css['bottom-bar-browse']}>
        <svg viewBox="0 0 18 18"><path d="M10 15v-5h5v5h-5zm0-12h5v5h-5V3zm-7 7h5v5H3v-5zm0-7h5v5H3V3z" /></svg>
        <p>Browse all</p>
      </div>

      <div className={css['photo-location']}>
        <p>{photoNum} of {photoTotalNum}</p>
      </div>

      <div className={css['bottom-bar-actions']}>
        <div className={css['bottom-bar-actions__container']}>
        ...
        <svg className={css['flag-icon']} viewBox="0 0 18 18"><path d="M6 10V3c4.976 1.098 4.024-1 8 0v7c-4.024-.976-3.024 1.024-8 0zM4 2h1v14H4V2z" /></svg>
      </div>
    </div>
  );
};
...
PhotoModalImageBottomBar.propTypes = {
  ...
  photoTotalNum: PropTypes.number.isRequired
};
